...
{
	{
		std::cout << "****************************  ƒобро пожаловать в Ѕриллиант!********************";
	...
	CreateStorage();
	{
		...
		std::string name[staticSize]{ "\t1-золота€ цепь, 2- серебр€нное кольцо, 3- золотое кольцо, 4- золотой слиток, 5-серебр€нный слиток, 6- подвеска" };
	...
	void Seling();
	{
		...
		void ChangePrice();
		{
			...
			double newprice = 0;
			do {
				std::cout << "введите id дл€ изменени€ цены";
				std::cin >> id;
		...
		void AddToStorage();
		{
			int id, count;
			do {
				std::cout << "введите id товара дл€ пополнени€";
				std::cin >> id;
		...
		void DeleteElementByIndex();
		{
			...
			int index = 0;
			do {
				std::cout << "введите id товара дл€ удалени€";
				std::cin >> index;
